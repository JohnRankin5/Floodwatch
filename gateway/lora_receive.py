...
while True:
    ...
    if packet is not None and len(packet) >= 4:
        ...
        try:
            ...
            print(f"Node {header_from} -> Node {header_to} (Seq: {header_id}): {payload_text}")
            display.text(f"N{header_from}->N{header_to} seq:{header_id}", 0, 0, 1)
            display.text(payload_text, 0, 10, 1)
        except UnicodeDecodeError:
            print(f"Node {header_from} -> Node {header_to} (Seq: {header_id}) [RAW]: {payload}")
            display.text(f"N{header_from}->N{header_to} seq:{header_id}", 0, 0, 1)
            display.text("RAW Bytes", 0, 10, 1)
