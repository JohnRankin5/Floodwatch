...
// -----------------------------------------------------------------------------
// Set this to 'true' to fake distance readings on nodes without physical
// sensors!
#define MOCK_SENSOR_MODE false

#define SENSOR_SERIAL Serial1
UltrasonicSensor waterSensor(SENSOR_SERIAL, MOCK_SENSOR_MODE);
...
// Mesh tracker: Prevents infinite looping storms by remembering the last heard
// ID
...
void loop() {
  ...
  if (rf95.available()) {
    ...
    if (rf95.recv(buf, &len)) {
      ...
      // MESH DEDUPLICATION CHECK: Have we seen this exact packet sequence
      // recently?
      if (seen_messages[from] != id) {
        seen_messages[from] = id; // Store it so we never relay it again

        // If this packet is NOT meant for us, and NOT a broadcast (255), we
        // must RELAY IT!
        if (to != 255 && to != MY_NODE_ID) {
          Serial.print(
              F("[REPEATER] Relaying a packet originating from Node "));
          Serial.print(from);
          Serial.print(F(" intended for Node "));
          Serial.println(to);

          // Force the radio headers to precisely impersonate the original
          // sender
          rf95.setHeaderTo(to);
  ...
  if (millis() - lastTransmitTime > TX_INTERVAL_MS) {
    ...
    char radiopacket[32];
    snprintf(radiopacket, sizeof(radiopacket), "Distance:%dmm",
             waterSensor.getDistance());
